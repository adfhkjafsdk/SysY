...
enum ASTGolbTag {
	AST_GT_FUNC, AST_GT_CONST, AST_GT_VAR
};

enum ASTStmtTag {
	AST_ST_VARDEF, AST_ST_CONSTDEF, AST_ST_EXPR, AST_ST_ASSIGN, AST_ST_BLOCK, 
	AST_ST_IF, AST_ST_ELSE, 
	AST_ST_WHILE, AST_ST_BREAK, AST_ST_CONTINUE,
	AST_ST_RETURN
};
...
public:
	std::vector<std::pair<ASTGolbTag, PtrAST> > glob_def;
...
class StmtIf: public BaseAST {		// if, while
public:
	PtrAST expr, stmt;
	PtrAST match;
	void Dump(std::ostream &out) const override;
